...
function normalizeExpense(input) {
  ...
  if (input.paid_for_artist) {
    const tokens = String(input.paid_for_artist).split(/[;,]/).map((t) => normalizeArtistName(t)).filter((t) => t.length > 0);
...
router.get("/api/yearly-summary", async (request, env) => {
  const url = new URL(request.url);
  const year = parseInt(url.searchParams.get("year") || "", 10);
  if (!year) {
  ...
  const monthlyExpenses = await queryAll(
    env,
  ...
  const monthlyShoots = await queryAll(
    env,
  ...
  const topExpenseCats = await queryAll(
    env,
  ...
  const growthPct = /* @__PURE__ */ __name((cur, prev) => {
...
// .wrangler/tmp/bundle-fGRUdt/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-fGRUdt/middleware-loader.entry.ts
